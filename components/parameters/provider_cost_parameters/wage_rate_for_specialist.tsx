...
const  WageRateForSpecialist: NextPage = () => {
  ...
  useEffect(() => {
    setSpecialistWageRate(window.localStorage.getItem('specialistWageRate') || '150.00');
    setLoading(false);
  }, []);
    ...
    <div className="grid mt-4 mx-4 px-6 py-6 md:mx-28 border rounded-xl border border-casal-300">
      <div className="flex justify-between md:pr-9">
        ...
        <div className="flex space-x-4 col-span-2 justify-self-end">
          ...
          <span className="self-center mt-2 text-casal-300 cursor-pointer select-none" onClick={() => setSpecialistWageRate('150.00')}>
            Set to default
          </span>
        </div>
      </div>
